...
import { 
  ...
  collection, getDocs, writeBatch, query, orderBy, onSnapshot
...
export function migrateDB(input) {
  ...
  if ((input.schemaVersion === 5 || input.schemaVersion === 4 || input.schemaVersion === 3 || input.schemaVersion === 2) && Array.isArray(input.trades)) {
    const db = {
      ...
      memos: (input.memos || []).map(normalizeMemo),
    };
...
function normalizeMeta(meta = {}) {
  ...
  return {
    ...
    macroBriefings: (meta.macroBriefings && typeof meta.macroBriefings === 'object' && !Array.isArray(meta.macroBriefings)) ? meta.macroBriefings : {},
...
/**
 * Firestore는 undefined 필드를 허용하지 않으므로, 객체에서 이를 재귀적으로 제거하거나 null로 변환합니다.
 */
...
/**
 * Firebase에서 전체 데이터를 가져오고 마이그레이션이 필요한지 확인합니다.
 */
export async function syncWithFirebase(user) {
  if (!user) return null;
  const uid = user.uid;

  try {
    const metaRef = doc(db, 'users', uid, 'config', 'main');
    const metaSnap = await getDoc(metaRef);

    if (!metaSnap.exists()) {
      console.log('[Firebase] No remote data found. Migrating local data...');
      const localDb = loadDB();
      await migrateLocalToFirebase(user, localDb);
      return localDb;
    }

    console.log('[Firebase] Remote data found. Fetching...');
    const remoteDb = await fetchFromFirebase(user);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(remoteDb));
    await idbSet(IDB_DB_KEY, remoteDb);
    return remoteDb;
  } catch (error) {
    console.error('[Firebase syncWithFirebase]', error);
    throw error;
  }
}

/**
 * Firestore에서 전체 데이터(Meta, Trades, Memos)를 가져옵니다.
 */
async function fetchFromFirebase(user) {
  const uid = user.uid;
  
  // Meta
  const metaSnap = await getDoc(doc(db, 'users', uid, 'config', 'main'));
  const meta = normalizeMeta(metaSnap.data() || {});

  // Trades
  const tradesSnap = await getDocs(query(collection(db, 'users', uid, 'trades'), orderBy('date', 'desc')));
  const trades = tradesSnap.docs.map(d => normalizeTrade({ id: d.id, ...d.data() }));

  // Memos
  const memosSnap = await getDocs(query(collection(db, 'users', uid, 'memos'), orderBy('date', 'desc')));
  const memos = memosSnap.docs.map(d => normalizeMemo({ id: d.id, ...d.data() }));

  return {
    schemaVersion: 5,
    _sync: { revision: 0, lastSavedAt: nowIso(), source: 'firebase' },
    meta,
    trades,
    memos
  };
...
/**
 * 로컬 데이터를 Firebase로 일괄 업로드합니다.
 * Firestore 배치 제한(500개)을 고려하여 청크 단위로 나누어 처리합니다.
 */
async function migrateLocalToFirebase(user, localDb) {
  const uid = user.uid;
  const BATCH_SIZE = 400; // 안전 마진을 위해 400개 단위로 처리
  
  // 1. Meta
  const metaRef = doc(db, 'users', uid, 'config', 'main');
  await setDoc(metaRef, cleanObject(localDb.meta));

  // 2. Trades
  const trades = localDb.trades || [];
  for (let i = 0; i < trades.length; i += BATCH_SIZE) {
    const batch = writeBatch(db);
    const chunk = trades.slice(i, i + BATCH_SIZE);
    chunk.forEach(trade => {
      const tradeRef = doc(db, 'users', uid, 'trades', trade.id);
      batch.set(tradeRef, cleanObject(trade));
    });
    await batch.commit();
    console.log(`[Firebase] Migrating trades... ${i + chunk.length}/${trades.length}`);
  }

  // 3. Memos
  const memos = localDb.memos || [];
  for (let i = 0; i < memos.length; i += BATCH_SIZE) {
    const batch = writeBatch(db);
    const chunk = memos.slice(i, i + BATCH_SIZE);
    chunk.forEach(memo => {
      const memoRef = doc(db, 'users', uid, 'memos', memo.id);
      batch.set(memoRef, cleanObject(memo));
    });
    await batch.commit();
    console.log(`[Firebase] Migrating memos... ${i + chunk.length}/${memos.length}`);
  }
...
/**
 * 개별 데이터 변경 시 Firebase에 즉시 반영하는 함수들
 */

export async function saveMetaToFirebase(user, meta) {
  if (!user) return;
  const metaRef = doc(db, 'users', user.uid, 'config', 'main');
  await setDoc(metaRef, cleanObject(meta));
}

export async function saveTradeToFirebase(user, trade) {
  if (!user) return;
  const tradeRef = doc(db, 'users', user.uid, 'trades', trade.id);
  await setDoc(tradeRef, cleanObject(trade));
}

export async function deleteTradeFromFirebase(user, tradeId) {
  if (!user) return;
...
/**
 * ⚡ Firebase Real-time Listener for Memos
 */
export function listenMemos(user, onUpdate) {
  if (!user) return null;
  const q = query(collection(db, 'users', user.uid, 'memos'), orderBy('date', 'desc'));
...
export function compressImage(base64Str, maxWidth = 1200, quality = 0.7) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // 3. Export as JPEG (most efficient for charts)
      const compressed = canvas.toDataURL('image/jpeg', quality);
      console.log(`[Storage] Compressed: ${(base64Str.length / 1024).toFixed(1)}KB -> ${(compressed.length / 1024).toFixed(1)}KB`);
      resolve(compressed);
    };
