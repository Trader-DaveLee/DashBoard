...
export function recalcTrade(trade) {
  ...
  if (filledExitPctRaw > 100) {
    const base = baseMetrics(pnlAdjustment, riskDollar, accountSize, 0, false, trade.manualPnL, trade.manualR);
  ...
  if (!stop || !entries.length || entryWeightTotal <= 0) {
    return baseMetrics(pnlAdjustment, riskDollar, accountSize, 0, false, trade.manualPnL, trade.manualR);
  }
  ...
  if (directionError || totalQty <= 0) return baseMetrics(pnlAdjustment, riskDollar, accountSize, 0, true, trade.manualPnL, trade.manualR);
  ...
  // 📝 Manual Override & Simplification (Manual R requested)
  const finalPnl = trade.manualPnL !== undefined ? trade.manualPnL : netPnl;
  const finalR = trade.manualR !== undefined ? trade.manualR : 0; // Forced manual R (default 0)
  ...
  return {
    ...
    grossRealized, realizedPnl, grossUnrealized, unrealizedPnl, pnl: finalPnl, netPnl: finalPnl,
    realizedR, unrealizedR, r: finalR, totalFees, entryFees, exitFees, feePctOfGross,
    ...
    pnlAdjustment, manualPnL: trade.manualPnL, manualR: trade.manualR
  };
}
...
function baseMetrics(pnlAdjustment = 0, riskDollar = 0, accountSize = 0, avgEntry = 0, directionError = false, manualPnL, manualR) {
  const pnl = manualPnL !== undefined ? manualPnL : Number(pnlAdjustment || 0);
  const r = manualR !== undefined ? manualR : 0; // Forced manual R (default 0)
